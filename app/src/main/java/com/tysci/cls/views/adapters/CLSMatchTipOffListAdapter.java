...
public class CLSMatchTipOffListAdapter extends RecyclerView.Adapter<CLSMatchTipOffListAdapter.CLSMatchTipOffListViewHolder>{
    ...
    public void onBindViewHolder(CLSMatchTipOffListViewHolder holder, int position) {
        ...
        CommonUtils.setTextViewFormatString(holder.tvUserName, "球商APP作家:" + info.getFname(), "球商APP作家", Color.parseColor("#78a639"), 1f);
        holder.tvTipOffContent.setText(info.getCont());
        holder.tvTipOffTime.setText(info.getCtime());
        KLog.e("tip_count:"+info.getTipcount()+" wins:"+info.getWins()+" ror:"+info.getRor());
        int wins=0;
        if(!TextUtils.isEmpty(info.getWins())&&!info.getWins().equals("null")){
            wins= (int) (Float.parseFloat(info.getWins()) * 100);
        }
        float ror=0f;
        if(!TextUtils.isEmpty(info.getRor())&&!info.getRor().equals("null")){
            ror=Float.parseFloat(info.getRor());
        }
        String bettingInfo="投注场次:" + info.getTipcount() + " 亚盘:" +wins+"%"+" 盈利:" + String.format(Locale.getDefault(), "%.2f",ror) + "%";
